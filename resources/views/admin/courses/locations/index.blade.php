    ...
    <div class="w-full h-[88%] bg-gray-200 flex items-center justify-center">
        <div class="w-[90%] h-5/6 bg-white rounded-xl pt-3 flex flex-col items-center">
            <div class="w-[90%] h-2/5 flex items-center justify-between border-b">
                <a href="{{route('courses.location.create',compact('course'))}}" class="px-10 py-3 rounded-xl font-light text-white bg-gray-800">add {{$course->title}}'s location +</a>
                <h2 class="text-xl">{{$course->title}} locations</h2>
            </div>
                        ...
                        @if($location->is_active==1)
                            <tr>
                                <td class="text-center">
                                    <form action="{{route('courses.locations.destroy',compact('course','location'))}}" method="post">
                                        @csrf
                                        @method('delete')
                                ...
                                <td class="text-center">
                                    <form action="{{route('courses.location.edit',compact('course','location'))}}" method="get">
                                        @csrf
